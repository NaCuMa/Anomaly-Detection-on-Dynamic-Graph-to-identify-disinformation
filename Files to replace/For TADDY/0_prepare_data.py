...
def preprocessDataset(dataset):
    ...
    #----------------------------------------
        ...
        if dataset == 'btc_alpha':
            file_name = 'C:/Users/geode/TADDY/data/raw/' + 'soc-sign-bitcoinalpha.csv'
        elif dataset =='btc_otc':
            file_name = 'C:/Users/geode/TADDY/data/raw/' + 'soc-sign-bitcoinotc.csv'
        elif dataset =='email':
            file_name = 'C:/Users/geode/TADDY/data/raw/' + 'email-dnc.csv'
        elif dataset =='TGN':
            file_name = 'C:/Users/geode/TADDY/data/raw/' + 'TGN.csv'
...
# vertexs permet de faire le lien entre les indices traités et non traités
    ...
    pd.DataFrame(times).to_csv(f'C:/Users/geode/TADDY/data/for_TGN/' + dataset + '_times.csv',
                                       index=False)
    pd.DataFrame(vertexs).to_csv(f'C:/Users/geode/TADDY/data/for_TGN/' + dataset + '_vertexs.csv',
                                       index=False)
#----------------------------------------------------------------
    np.savetxt(
        'C:/Users/geode/TADDY/data/interim/' +
        dataset,
        X=edges,
        delimiter=' ',
        comments='%',
...
def generateDataset(dataset, snap_size, train_per=0.5, anomaly_per=0.01):
    ...
    if not os.path.exists('C:/Users/geode/TADDY/data/interim/' + dataset):
...
# soit de reconstruire un jeu de données comparable à celui utilisé par TADDY mais adapté à TGN
    Path('C:/Users/geode/TADDY/data/for_TGN/').mkdir(parents=True, exist_ok=True)
    pd.DataFrame(train).to_csv(f'C:/Users/geode/TADDY/data/for_TGN/' + 'train_' + dataset + '_' + str(train_per) + '_' + str(anomaly_per) +'.csv',
                                       index=False)
    pd.DataFrame(synthetic_test).to_csv(f'C:/Users/geode/TADDY/data/for_TGN/' + 'test_' + dataset + '_' + str(train_per) + '_' + str(anomaly_per) +'.csv',
                                       index=False)
#------------------------------------------------------------------
...
# Cela permet également de déterminer l'instant jusqu'auquel entraîner TGN.
    ...
    Path('C:/Users/geode/TADDY/data/for_TGN/').mkdir(parents=True, exist_ok=True)
    pd.DataFrame(snap_list).to_csv(f'C:/Users/geode/TADDY/data/for_TGN/' + dataset + '_' + str(train_per) + '_' + str(anomaly_per) +'_snap_indices.csv',
                                       index=False)
#------------------------------------------------------------------

    with open('C:/Users/geode/TADDY/data/percent/' + dataset + '_' + str(train_per) + '_' + str(anomaly_per) + '.pkl', 'wb') as f:
